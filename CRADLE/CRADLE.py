...
def myMain(planLibrary, observations):
    ...
    exps = ExplainAndCompute(planLibrary, observations)
    exps.sort(key=Explanation.Explanation.getExpProbability)
    most_likely_exp = exps.pop()
    tree = most_likely_exp.getTree(0)
    task_node = tree.getRoot()
    task_name = task_node._ch
    task_parameter = task_node._params[0][1]
    print("Human is doing: {0} {1}".format(task_name, task_parameter))
    target = tree.getFrontier(withIndices=False)
    target_node = target[0].getRoot()
    target_name = target_node._ch
    target_parameter = target_node._params[0][1]
    print("Robot should do: {0} {1}\n".format(target_name, target_parameter))
